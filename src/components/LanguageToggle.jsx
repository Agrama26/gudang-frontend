...
const LanguageToggle = ({ className = "" }) => {
  ...
  return (
    ...
    >
      ...
      {/* Toggle circle */}
      <div className={`relative z-10 flex items-center justify-center w-7 h-7 bg-white dark:bg-gray-100 rounded-full shadow-lg transform transition-all duration-300 ${isIndonesian ? '-translate-x-3.5' : 'translate-x-3.5'
        }`}>
        {/* ID Text - Indonesian */}
        <span
          className={`absolute inset-0 flex items-center justify-center text-xs font-bold transition-all duration-300 ${isIndonesian
      ...
      <div className="absolute inset-0 flex items-center justify-between px-2 pointer-events-none">
        <span className={`text-[10px] font-semibold transition-all duration-300 ${isIndonesian
